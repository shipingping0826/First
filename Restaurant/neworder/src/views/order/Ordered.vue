	...
	<el-scrollbar wrap-class="scrollWrap" :native="false" id="content">
		...
		<ul class="orderBox" v-if="ordered.length">
			<li v-for="(table,tableIndex) in ordered" :key="table.table_id">
				<table>
					<tbody>
						<tr v-for="(dish,dishIndex) in table.order_items" :key="dish.dish_id">
							<td>{{dishIndex+1}}</td>
							<td><img :src="'http://order-api.chenxuehui.com/'+dish.image" alt=""></td>
							<td>{{dish.name}}</td>
							<td>
				...
				<div class="foot">
					<div>
						...
						<button @click="addDishes(table.table_id)" class="addDishes">添菜</button>
					</div>
					<div>
						<span>总消费：￥<strong>{{totalPrice(tableIndex)}}</strong></span>
						<button @click="submitOrder(table)" class="pay">下单</button>
					</div>
				</div>
			</li>
		</ul>
...
export default {
	computed:{
		...
		totalPrice(){	//计算总价
			return function(tableIndex){
				let total=0;
				this.ordered[tableIndex].order_items.forEach(dish => total += dish.price * dish.num)
	...
	methods:{
		addOrDel(dish,num,table_id){	//加减菜
			dish.num+=num;
			dish.selected=true;	
		...
		orderDishes(dish,table_id){	//这里请求后不需要再发获取已点数据的请求
			this.$fetch({
		...
		addDishes(table_id){	//添菜
			if(table_id!=this.curTableId){	//当前点的桌子id与当前桌子id不一致
				this.$fetch({
