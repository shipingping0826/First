	...
	<div>
		...
		<ul class="foodList">
			<li v-for="food in foodList" :key="food.dish_id">
				<img :src="'http://order-api.chenxuehui.com/'+food.image" alt="" @click="showDetail(food)">
		...
		<Detailed v-if="detailedShow" @closeDetailed='detailedShow=false' :food="curFood" />

		<el-dialog :title="curFood.name" :visible.sync="dialogVisible" :modal-append-to-body="false" width="30%" top="30vh" custom-class="attrDialog">
			<div class="dishHead">
				<div class="title">
					<p class="price">￥<strong>{{curFood.price}}</strong></p>
					<p class="star">
						<el-rate v-model="curFood.stars" disabled text-color="#ff9900"></el-rate>
					</p>
				</div>
				<div class="control">
			...
			<el-input type="textarea" rows="8" placeholder="请输入备注" v-model="curFood.remarks"></el-input>
...
export default {
	...
	methods: {
		...
		showFoodAttr(food){
			if(!this.tableId){	//这个条件成立，说明用户没有选座，需要跳转到选座页面
				this.$alert('请选择座位').then(()=>this.$router.push('/table')).catch(()=>{});
				return;
			}
			...
			const orderTable=this.ordered.find(table=>table.table_id==this.tableId);	//先查桌子有没有点餐
			const orderDish=orderTable && orderTable.order_items.find(dish=>dish.dish_id==food.dish_id);	//再查这道有没有点过（如果桌子在已点里没有找到，就会返回undefined，所以查菜的前提条件是桌子必需点过菜，所以用了&&）
			
			if(orderDish){	//走到这里说明这道菜点过了
